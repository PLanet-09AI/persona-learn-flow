...
import { Trophy, Medal, Award, Star, BookOpen, TrendingUp, Loader2 } from "lucide-react";
...
import type { LeaderboardUser, Achievement } from "@/services/leaderboard";
...
export const Leaderboard = ({ userProfile, userId, onNewLesson }: LeaderboardProps) => {
  const [leaderboardData, setLeaderboardData] = useState<LeaderboardUser[]>([]);
  const [achievements, setAchievements] = useState<Achievement[]>([]);
  const [isLoading, setIsLoading] = useState(true);

  // Update leaderboard when userProfile or userId changes
  useEffect(() => {
    const loadData = async () => {
      setIsLoading(true);
      try {
        console.log('📊 Loading leaderboard for user:', userId);
        
        const leaderboard = await getLeaderboardWithUserVisible(userProfile, userId);
        setLeaderboardData(leaderboard);

        const userAchievements = await getUserAchievements(userProfile, userId);
          ...
          <Card className="border-primary/20 bg-primary/5">
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="text-center">
                  <div className="text-2xl font-bold text-primary">#{leaderboardData.find(u => u.isCurrentUser)?.rank || 0}</div>
                  <div className="text-sm text-muted-foreground">Current Rank</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-foreground">{leaderboardData.find(u => u.isCurrentUser)?.score || 0}</div>
                  <div className="text-sm text-muted-foreground">Total Points</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-foreground">{leaderboardData.find(u => u.isCurrentUser)?.level || 0}</div>
                  <div className="text-sm text-muted-foreground">Level</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-foreground">{leaderboardData.find(u => u.isCurrentUser)?.streak || 0}</div>
